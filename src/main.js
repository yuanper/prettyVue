...
import { Button, Row, Col, Search, Swipe, SwipeItem, Lazyload, Tabbar, 
              ...
              Stepper, Step, Steps, Icon, Field, PullRefresh, GoodsAction, GoodsActionBigBtn, GoodsActionMiniBtn, RadioGroup, Radio,
  AddressList, Area, AddressEdit,Dialog } from 'vant'
...
Vue.use(Button).use(Row).use(Col).use(Search).use(Swipe).use(SwipeItem).use(Lazyload).use(VueAwesomeSwiper)
  ...
  .use(Stepper).use(Step).use(Steps).use(Icon).use(Field).use(PullRefresh).use(GoodsAction).use(GoodsActionBigBtn).use(GoodsActionMiniBtn)
  .use(RadioGroup).use(Radio).use(AddressList).use(Area).use(AddressEdit).use(Dialog)
